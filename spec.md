...
- Floating monitor shows only configured accounts and ready metrics, sizes text so it is not clipped, and scales from an offscreen bitmap when the user resizes its width. Usage rows appear only after a metric has a value to display. Authentication failure keeps the last known usage on the monitor instead of clearing the account, tints that entire provider row orange, and opens that provider's connect flow when the row is clicked. Rows that consumed allowance since the last poll are brighter; idle rows are darker.
